...
HESSIAN_MEAN_DIR = "cache/hessians_per_layer_mean" # per-layer mean (new)
QUANTIZED_DIR = "cache/quantized_per_layer_H"      # NEW output dir
...
def _load_layer_mean_hessian(layer, kind):
    ...
    path = os.path.join(HESSIAN_MEAN_DIR, f"L{layer:02d}", f"layer_mean_{kind}.pt")
    return torch.load(path, weights_only=True)["H"].numpy()
